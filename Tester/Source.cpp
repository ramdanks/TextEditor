...
bool worker( const filesystem::path& filepath )
{
    ...
    auto vComp = Compression::Compress(&vRead[0], vRead.size(), false);
    auto vRes = Compression::Decompress(vComp, false);

    if (vRead.size() == vRes.OriginalSize &&
        memcmp(&vRes.pData[0], &vRead[0], vRead.size()) == 0)
    {
        wprintf(L"[PASS]:%ls\n", filepath.c_str());
        return true;
    }
